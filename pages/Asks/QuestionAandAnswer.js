...
export const LoadingElement = () => {
    return <div ><span role="progressbar" aria-labelledby=":r2s:" style="width: 16px; height: 16px;"><svg viewBox="22 22 44 44"><circle cx="44" cy="44" r="20.2" fill="none" strokeWidth="3.6"></circle></svg></span></div>
}
export default function QuestionAandAnswer({ question }) {
    ...
    useEffect(() => {
        ...
        setLoading(true)
        API("MyQuestions", { Question: question }).then((answer) => {
            setLoading(false)
    ...
    return <div key="left-panel" className={"flex flex-col justify-between min-w-250 w-full bg-zinc-200 border-amber-200 rounded-xl text-base h-full"}>
        <div key="list-of-topics" className="flex flex-col h-full w-full" >
            {!!QA.Q && <div className="flex flex-row justify-center w-full bg-gray-50 flex-nowrap">{ }
                ...
                <div key={`qa-q${QA?.Q}`} className="hover:bg-amber-200 flex-row gap-10 items-center p-10 rounded-10 self-start   max-w-2xl w-full" >
                    {QA?.Q}
                </div>
            </div>
            }
            {!!QA?.A && <div className="flex flex-row justify-center  w-full  bg-gray-200 flex-nowrap">
                ...
                <div key={`qa-a${QA?.A}`} className="hover:bg-amber-200 flex-row gap-10 items-center p-10 rounded-10 self-start   max-w-2xl w-full h-full" >
                    {loading ? <LoadingElement /> : QA?.A}
                </div>
            </div>}
        </div>
        ...
        <div className="flex flex-col w-full  mb-2" >
            <div key="question-box" className="flex flex-col w-full py-2 flex-grow md:py-3 md:pl-4 relative border border-black/10 bg-white dark:border-gray-900/50 dark:text-white dark:bg-gray-700 rounded-md shadow-[0_0_10px_rgba(0,0,0,0.10)] dark:shadow-[0_0_15px_rgba(0,0,0,0.10)] max-w-2xl self-center "  >
                ...
                <button className="absolute p-1 rounded-md text-gray-500 bottom-1.5 md:bottom-2.5 hover:bg-gray-100 dark:hover:text-gray-400 dark:hover:bg-gray-900 disabled:hover:bg-transparent dark:disabled:hover:bg-transparent right-1 md:right-2" onClick={e => OnSubmitQuestion(question)}>
                    {loading ? <LoadingElement /> : <svg stroke="currentColor" fill="none" strokeWidth="2" viewBox="0 0 24 24" strokeLinecap="round" strokeLinejoin="round" className="h-4 w-4 mr-1" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg"><line x1="22" y1="2" x2="11" y2="13"></line><polygon points="22 2 15 22 11 13 2 9 22 2"></polygon></svg>}
                </button>
            </div>
        </div>
    </div>
