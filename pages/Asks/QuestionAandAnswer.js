...
export default function QuestionAandAnswer() {
    ...
    const OnSubmitQuestion = (Question) => {
        setLoading("⌛")
        API("MyQuestions", { Question }).then((answer) => {
            setLoading("")
    ...
    return <div key="left-panel" className={"flex flex-col justify-between min-w-250 w-full bg-zinc-200 border-amber-200 rounded-xl text-base h-full overflow-y-scroll"}>
        ...
        <div className={`flex flex-col w-full  mb-2 mt-1 ${loading && "animate-pulse"}`} >
            <div key="question-box" className="flex flex-row w-full py-2 flex-grow md:py-3 md:pl-4  border border-black/10 bg-white dark:border-gray-900/50 dark:text-white dark:bg-gray-700 rounded-md shadow-[0_0_10px_rgba(0,0,0,0.10)] dark:shadow-[0_0_15px_rgba(0,0,0,0.10)] max-w-2xl self-center "  >
