...
export default function QuestionAandAnswer() {
    ...
    const OnSubmitQuestion = (Question) => {
        ...
        API("MyQuestions", { Question, Model: modelGPT }).then((answer) => {
            ...
            if (!answer?.Q || !answer?.A || !answer?.Time) return setOpenAlert("答案返回失败，请重试")
            //clear question
            setQuestion("")
            setQA(answer)
    ...
    return <div key="left-panel" className={"flex flex-col justify-between min-w-250 w-full bg-zinc-200 border-amber-200 rounded-xl text-base h-full overflow-y-scroll"}>
        <div key="list-of-topics" className="flex flex-col h-full w-full " >
            ...
            {!!QA?.Q && <div className="flex flex-row justify-center w-full bg-gray-50 flex-nowrap overflow-visible  ">{ }
        ...
        {!QA.Q && <div className={`flex flex-col w-full   mb-2 mt-1 ${loading && "animate-pulse"}`} >
            <div key="question-box" className="flex flex-row w-full h-full py-2 flex-grow md:py-3 md:pl-4  border border-black/10 bg-white dark:border-gray-900/50 dark:text-white dark:bg-gray-700 rounded-md shadow-[0_0_10px_rgba(0,0,0,0.10)] dark:shadow-[0_0_15px_rgba(0,0,0,0.10)] max-w-2xl self-center items-center overflow-auto "  >
                <textarea id="question-ask" className="flex flex-row m-0 w-full items-center resize-none border-0 bg-transparent p-0 pr-7 focus:ring-0 focus-visible:ring-0 dark:bg-transparent pl-2 md:pl-0 outline-none  h-auto " rows={1}
                    ...
                    onChange={(e) => {
                        ...
                        //auto set height of question-ask to it's scroll height
                        e.target.style.height = "auto"
                        //change rows according to scroll height
                        e.target.rows = Math.min(Math.max(e.target.scrollHeight / 20, 1), 5)
                        //auto decrease height if content is deleted
                ...
                {loading ? <div key={`loading-${loading}`} className="flex flex-row p-1 rounded-md text-gray-500 bottom-1.5 md:bottom-2.5 hover:bg-gray-100 dark:hover:text-gray-400 dark:hover:bg-gray-900 disabled:hover:bg-transparent dark:disabled:hover:bg-transparent right-1 md:right-2">{loading}</div> :
                    <button className="flex  p-1 rounded-md text-gray-500 bottom-1.5 md:bottom-2.5 hover:bg-gray-100 dark:hover:text-gray-400 dark:hover:bg-gray-900 disabled:hover:bg-transparent dark:disabled:hover:bg-transparent right-1 md:right-2" onClick={e => OnSubmitQuestion(question)}>
